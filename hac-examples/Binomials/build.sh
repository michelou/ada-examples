...
echo "00000000000000000000000 $HAT_DIR"
    local main_file="$SOURCE_MAIN_DIR/$MAIN_NAME.adb"
    ## -we : Treat all warnings as errors
    local gnatmake_opts="-gnat2022 -we -I$(mixed_path $HAT_DIR/src/main/ada) -D \"$TARGET_OBJ_DIR\" -o \"$TARGET_FILE\""
...
HAT_DIR="$(dirname $ROOT_DIR)/HAT"
if [[ ! -d "$HAT_DIR" ]]; then
    error "HAT Directory not found"
    cleanup 1
fi
