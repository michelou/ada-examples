...
set _HAT_DIR=
for /f "delims=" %%f in ("%~dp0.") do set "_PARENT_DIR=%%~dpf"
if not exist "%_PARENT_DIR%HAT" (
    echo %_ERROR_LABEL% HAT Directory not found 1>&2
    set _EXITCODE=1
    goto :eof
)
set "_HAT_DIR=%_PARENT_DIR%HAT"
...
set __N=0
for /f "delims=" %%f in ('dir /b /s "%_SOURCE_MAIN_DIR%\*.ad?" "%_HAT_DIR%\src\main\ada\*.ad?" 2^>NUL') do (
    set /a __N+=1
)
if %__N%==0 (
...
set "__MAIN_FILE=%_SOURCE_MAIN_DIR%\%_MAIN_NAME%.adb"
@rem -we : Treat all warnings as errors
set __GNATMAKE_OPTS=-gnat2022 -we -I%_HAT_DIR%\src\main\ada -D "%_TARGET_OBJ_DIR%" -o "%_EXE_FILE%"
if %_DEBUG%==1 set __GNATMAKE_OPTS=-d %__GNATMAKE_OPTS%

if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%__GNATMAKE_CMD%" %__GNATMAKE_OPTS% %__MAIN_FILE% 1>&2
) else if %_VERBOSE%==1 ( echo Compile %__N_FILES% into directory "!_TARGET_OBJ_DIR:%_ROOT_DIR%=!" 1>&2
)
call "%__GNATMAKE_CMD%" %__GNATMAKE_OPTS% %__MAIN_FILE% %_STDERR_REDIRECT%
if not %ERRORLEVEL%==0 (
