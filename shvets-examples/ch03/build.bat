...
set _MAIN_NAME_DEFAULT=for_loop
...
:args_main
@rem TODO: check if source file exists with name %~1
set _ARGS_MAIN=%~1
goto :eof
...
for /f %%f in ('dir /b "%_SOURCE_MAIN_DIR%\*.adb"') do (
    set __FILE_NAME=%%f
    if defined __MAIN_NAMES ( set __MAIN_NAMES=!__MAIN_NAMES!, !__FILE_NAME:~0,-4!
    ) else ( set __MAIN_NAMES=!__FILE_NAME:~0,-4!
    )
)
echo   %__BEG_P%Variables:%__END%
echo     %__BEG_O%name=%__MAIN_NAMES%%__END%
goto :eof
...
set __GNATMAKE_OPTS=-gnat2022 -we -D "%_TARGET_OBJ_DIR%" -o "%_TARGET_DIR%\%_EXE_NAME%.exe"
