...
set __N=0
for /f "delims=" %%f in ('dir /s /b "%_SOURCE_MAIN_DIR%\*.adb" "%_SOURCE_MAIN_DIR%\*.ads" 2^>NUL') do (
...
if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_GNATMAKE_CMD%" "%_SOURCE_MAIN_DIR%\%_MAIN_NAME%.adb" -D "%_TARGET_OBJ_DIR%" -o "%_TARGET_DIR%\%_EXE_NAME%.exe" 1>&2
) else if %_VERBOSE%==1 ( echo Compile %__N_FILES% to object directory "!_TARGET_OBJ_DIR:%_ROOT_DIR%=!" 1>&2
)
call "%_GNATMAKE_CMD%" "%_SOURCE_MAIN_DIR%\%_MAIN_NAME%.adb" -D "%_TARGET_OBJ_DIR%" -o "%_TARGET_DIR%\%_EXE_NAME%.exe" %_STDERR_REDIRECT%
if not %ERRORLEVEL%==0 (
