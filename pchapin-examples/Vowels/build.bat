...
set __TEXT_INPUT=heed hid head head haired had
if %_DEBUG%==1 ( echo %_DEBUG_LABEL% echo %__TEXT_INPUT% ^| "%_EXE_FILE%" %_MAIN_ARGS% 1>&2
) else if %_VERBOSE%==1 ( echo Execute Ada program "!_EXE_FILE:%_ROOT_DIR%=!" 1>&2
)
echo %__TEXT_INPUT% | call "%_EXE_FILE%" %_MAIN_ARGS%
if not %ERRORLEVEL%==0 (
