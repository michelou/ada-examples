...
run() {
    ...
    if [[ $DEBUG -eq 1 ]]; then
        debug "$TARGET_FILE $MAIN_ARGS"
    elif [[ $VERBOSE -eq 1 ]]; then
        echo "Execute \"${TARGET_FILE/$ROOT_DIR\//}\"" 1>&2
    fi
    eval "$TARGET_FILE" $MAIN_ARGS
    if [[ $? -ne 0 ]]; then
